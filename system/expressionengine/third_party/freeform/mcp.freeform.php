...
class Freeform_mcp extends Module_builder_freeform
{
	...
	public function __construct( $switch = TRUE )
	{
		...
		$menu	= array(
			...
			'module_email_logs'				=> array(
				'link'  => $this->base . AMP . 'method=email_logs',
	...
	public function email_logs( $message = '' )
	{
		...
		$this->set_highlight('module_email_logs');
		...
		if ( ! empty($email_logs))
		{
			foreach($email_logs as $key => $log)
			{
				...
				$email_logs[$key]['type'] = ucfirst($log['type']);

				$email_logs[$key]['success'] = (
					$log['success'] == 'y'
				) ? lang('yes') : lang('no');

				$email_logs[$key]['site_label'] = $sites[$log['site_id']];

				$email_logs[$key]['message'] = str_replace(
					"\n",
					"<br/>",
		...
		$this->cached_vars['email_logs'] = $email_logs;

		// -------------------------------------
		//	messages
		// -------------------------------------

		$this->cached_vars['clear_link'] = $this->mod_link(array(
			'method' => 'clear_email_logs'
		));

		$this->cached_vars['logging_enabled'] = $this->check_yes($this->preference('email_logging'));

		$this->cached_vars['disabled_message'] = str_replace(
			'%preferences%',
			'<a href="' . $this->mod_link(array('method' => 'preferences')) .
				'">' . lang('preferences') . '</a>',
			lang('email_log_not_enabled')
		);

		$this->cached_vars['enabled_message'] = str_replace(
			'%preferences%',
			'<a href="' . $this->mod_link(array('method' => 'preferences')) .
				'">' . lang('preferences') . '</a>',
